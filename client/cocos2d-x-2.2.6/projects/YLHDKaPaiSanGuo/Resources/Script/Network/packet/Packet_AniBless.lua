--选择赐福战队
module("Packet_AniBless",package.seeall)
...
function CreatePacket( nType,nIndex )
	...
	pNetStream:Write(C_MS_NET_MSG_ID.C_MS_ANIMAL_BLESS )
	pNetStream:Write(0)
	pNetStream:Write(0)
...
function Server_Excute( PacketData )
	...
	if status == 1 then
		...
		elseif errorCode == 1010 then
			...
			pTips = nil
		elseif errorCode == 1080 then
			NetWorkLoadingLayer.loadingHideNow()
			local pTips = TipCommonLayer.CreateTipLayerManager()
			pTips:ShowCommonTips(1080,nil)
			pTips = nil
		elseif errorCode == 1230 then
			NetWorkLoadingLayer.loadingHideNow()
			local pTips = TipCommonLayer.CreateTipLayerManager()
			pTips:ShowCommonTips(1230,nil)
			pTips = nil
		else
			NetWorkLoadingLayer.loadingHideNow()
...
PacketManage.RegistPacketFun(NET_MSG_TYPE_ID.TYPE_MS_C,MS_C_NET_MSG_ID.MS_C_ANIMAL_BLESS ,Server_Excute)