...
module("CorpsMercenaryLayers",package.seeall)
...
local function _Click_return_CallBack( sender,eventType )
	if eventType == TouchEventType.ended then 
		...
		m_CorpsMercearyLayer = nil
		if CorpsLogic.CheckMerStatus(tableHaveMercenaryDB) == false then
			CorpsScene.DeleteRedPoint(5)
		elseif CorpsLogic.CheckMerStatus(tableHaveMercenaryDB) == true then
			CorpsScene.upDatePointMercenary()
		end
...
--左上
function showUpLeft()
	local Panel_upLeft = tolua.cast(m_CorpsMercearyLayer:getWidgetByName("Panel_top"),"Layout")
	local img_topBg = tolua.cast(Panel_upLeft:getChildByName("Image_bg"),"ImageView")
	--label 所拥有的佣兵数量
	local label_MencerNum = tolua.cast(img_topBg:getChildByName("Label_my"),"Label")
	local label_topContent = tolua.cast(img_topBg:getChildByName("Label_content1"),"Label")
	...
	label_MencerNum:setText("我的佣兵："..num.."/"..TotalHireNum)

	local nMessText2 = "|color|51,25,13||size|24|".."雇佣兵将在早4:00点自动离开您的队伍"
	if label_topContent:getChildByTag(10) ~= nil then
	else
		local messContentItem2 = RichLabel.Create(nMessText2,250,nil,nil,1)
		messContentItem2:setPosition(ccp(-115,35))		
		label_topContent:addChild(messContentItem2,10,10)
	end

	local btn_return = tolua.cast(m_CorpsMercearyLayer:getWidgetByName("Button_return"),"Button")
...
local function MercenaryInfoControl( pControl, value)
	...
	if value ~= nil then
		...
		if img_Rebate:getChildByTag(102) ~= nil then
			img_Rebate:getChildByTag(102):setText(dis_countNum.."折")
		else
			...
			img_Rebate:addChild(label_rebate,102,102)
		end
...
local function InitInfoControl( pControl ,value)
	...
	local m_touch = tolua.cast(pControl:getChildByName("Image_63"),"ImageView")
	...
	local img_lockbtom = tolua.cast(Panel_tianjia:getChildByName("Image_baikuang"),"ImageView")
	...
	if HavePtag == 1 then
		l_levelLimitWord:setText(GridLevel.."级解锁")
	elseif HavePtag == 2 then
		local vipF = MainScene.CheckVIPFunction(enumVIPFunction.eVipFunction_5)
		l_levelLimitWord:setText(vipF["level"].."级".."或VIP"..vipF["vipLevel"].."级解锁")
	...
	if next(value) == nil then
		...
		if HavePtag == 1 then
			if tonumber(n_pLevel) >= tonumber(GridLevel)  then --or tonumber(VIPLevel) >= tonumber(needVIP)
				...
				if next(value) == nil then
					if img_lockbtom:getChildByTag(20) == nil then
						m_PointManger:ShowRedPoint(20,img_lockbtom,55,50)
					end
				else
					m_PointManger:DeleteRedPoint(20)
				end
			end 
			img_vip:setVisible(false)
		elseif HavePtag == 2  then
			...
			if tonumber(n_pLevel) >= tonumber(GridLevel) or tonumber(vipF["vipLimit"]) >= 1 then --
				...
				l_levelLimitWord:setText("虚席以待")
				if next(value) == nil then
					if img_lockbtom:getChildByTag(21) == nil then
						m_PointManger:ShowRedPoint(21,img_lockbtom,55,50)
					end
				else
					m_PointManger:DeleteRedPoint(21)
				end
			end
		elseif HavePtag == 3  then
			...
			if tonumber(n_pLevel) >= tonumber(GridLevel) or tonumber(vipF["vipLimit"]) >= 1 then --
				...
				l_levelLimitWord:setText("虚席以待")
				if next(value) == nil then
					if img_lockbtom:getChildByTag(22) == nil then
						m_PointManger:ShowRedPoint(22,img_lockbtom,55,50)
					end
				else
					m_PointManger:DeleteRedPoint(22)
				end
			end
		end
		...
		if HavePtag == 1 then
			if img_lockbtom:getChildByTag(20) ~= nil then
				img_lockbtom:getChildByTag(20):removeFromParentAndCleanup(true)
			end
		elseif HavePtag == 2 then
			if img_lockbtom:getChildByTag(21) ~= nil then
				img_lockbtom:getChildByTag(21):removeFromParentAndCleanup(true)
			end
		elseif HavePtag == 3 then
			if img_lockbtom:getChildByTag(22) ~= nil then
				img_lockbtom:getChildByTag(22):removeFromParentAndCleanup(true)
			end
			-- m_PointManger:DeleteRedPoint(22)
		end
...
function showYetSolider()
	local Panel_YetSolider = tolua.cast(m_CorpsMercearyLayer:getWidgetByName("Panel_top"),"Layout")
	local img_TopBg = tolua.cast(Panel_YetSolider:getChildByName("Image_bg"),"ImageView")

	local pImage_item1 = tolua.cast(img_TopBg:getChildByName("Image_item1"),"ImageView")
	local pImage_item2 = tolua.cast(img_TopBg:getChildByName("Image_item1_0"),"ImageView")
	local pImage_item3 = tolua.cast(img_TopBg:getChildByName("Image_item1_1"),"ImageView")
	HavePtag = 1
	vipLimitNum = 4
	m_disRewen = 0
	tableMercenaryRobotDB = {}
...
local function InitBottomInfoControl( pControl,value )
	...
	if next(value) ~= nil then
		...
		--显示是否已经被雇佣
		if CheckWhetherOwn(value.onlyID,tableHaveMercenaryDB) == true then
		...
		if label_cost:getChildByTag(100) ~= nil then
			LabelLayer.setText(label_cost:getChildByTag(100),costNum)
		else
			local labelCostText = LabelLayer.createStrokeLabel(24, CommonData.g_FONT3, costNum, ccp(0, 0), ccc3(83,28,2), ccc3(255,245,126), true, ccp(0, -2), 2)
			label_cost:addChild(labelCostText,100,100)
		end

		--折扣显示

		local dis_countNum = tonumber(dis_count)*10
		if image_rebate:getChildByTag(101) ~= nil then
			image_rebate:getChildByTag(101):setText(dis_countNum.."折")
		else
			local label_rebate = Label:create()
			label_rebate:setFontSize(18)
			label_rebate:setRotation(-45)
			...
			label_rebate:setText(dis_countNum.."折")
			label_rebate:setPosition(ccp(-5,6))
			image_rebate:addChild(label_rebate,101,101)
		end
		if tonumber(dis_count) == 1 then
...
local function showMiddle()
	if m_CorpsMercearyLayer == nil then
		return
	end
	...
	local img_consum = tolua.cast(Panel_middle:getChildByName("Image_consum"),"ImageView")
	img_consum:setVisible(false)
	...
	if n_curfreeNum <= 0 then
		n_curfreeNum = 0
		img_consum:setVisible(true)
		label_free:setText("刷新消耗：")
		label_freeNum:setText(RefConMNum)
	else
	...
	if label_content:getChildByTag(10) == nil then
		local messContentItem1 = RichLabel.Create("|color|233,180,114||size|18|"..m_tableLevelInfo.TechDes,500,nil,nil,1)
		messContentItem1:setPosition(ccp(-250,25))		
		label_content:addChild(messContentItem1,10,10)
	end
	...
	local function RefreshLayer(  )
		if m_refreshLayer == nil then
			...
			scenetemp:addChild(m_refreshLayer, 122, 122)
		end
	...
	if btn_refresh:getChildByTag(103) ~= nil then
		LabelLayer.setText(btn_refresh:getChildByTag(103),"刷新")
	else
		local labelReshText = LabelLayer.createStrokeLabel(24, CommonData.g_FONT1, "刷新", ccp(0, 0), ccc3(83,28,2), ccc3(255,245,126), true, ccp(0, -2), 2)
		btn_refresh:addChild(labelReshText,103,103)
	end
...
function showMercenaryLayer(tabMerLevel)
	...
	m_CorpsMercearyLayer:addWidget( GUIReader:shareReader():widgetFromJsonFile("Image/CorpsMercenaryLayers.json"))
	
	SetRefreshStatus(m_isRefresh)
	showUI()

	--测试
	--[[local vipFf = MainScene.CheckVIPFunction(enumVIPFunction.eVipFunction_11)
	printTab(vipFf)
	Pause()]]--
