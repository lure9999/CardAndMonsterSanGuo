...
public:
    ...
    void resetPayState();
	//add by sxin 增加支付数据接口
	void pay(const char* Product_Price,const char* Product_Id,const char* Product_Name,const char* Server_Id,const char* Product_Count,const char* Role_Id,const char* Role_Name,const char* Role_Grade,const char* Role_Balance);
...
private:
	...
	//add by sxin 回调指针
	//add by sxin anysdk回调
	void AnySDK_LoginSuccess();
	void AnySDK_LoginFail();
	void AnySDK_InitSuccess();
	void AnySDK_onRequestResult();
	void AnySDK_onPayResult_Success();	
};
