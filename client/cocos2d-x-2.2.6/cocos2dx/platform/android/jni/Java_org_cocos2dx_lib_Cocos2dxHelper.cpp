...
void showEditTextDialogJNI(const char* pszTitle, const char* pszMessage, int nInputMode, int nInputFlag, int nReturnType, int nMaxLength, EditTextCallback pfEditTextCallback, void* ctx) {
	...
	if (JniHelper::getStaticMethodInfo(t, CLASS_NAME, "showEditTextDialog", "([BLjava/lang/String;IIII)Z")) 
	{

		int count = strlen(pszMessage);
		jbyteArray strArray = t.env->NewByteArray(count);
		t.env->SetByteArrayRegion(strArray, 0, count, reinterpret_cast<const jbyte*>(pszMessage));
       ...
       // t.env->CallStaticVoidMethod(t.classID, t.methodID, stringArg1, stringArg2, nInputMode, nInputFlag, nReturnType, nMaxLength);
		...
		if (!t.env->CallStaticBooleanMethod(t.classID, t.methodID, strArray, stringArg1, nInputMode, nInputFlag, nReturnType, nMaxLength))
		{
			return;
		}		
