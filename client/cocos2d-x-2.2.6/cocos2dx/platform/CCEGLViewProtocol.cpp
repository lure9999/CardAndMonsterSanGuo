...
void CCEGLViewProtocol::setDesignResolutionSize(float width, float height, ResolutionPolicy resolutionPolicy)
{
	...
	//add by sxin
	if (resolutionPolicy == kResolutionFixedStandard_4s)
	{
		//add by sxin 全屏标准参照4s适配
		float f_4s_with = 960.0f;
		float f_4s_Height = 640.0f;

		if (m_obScreenSize.width/f_4s_with < m_obScreenSize.height/f_4s_Height)//以长适配
		{

			m_fScaleX = m_fScaleY = (float)m_obScreenSize.width / f_4s_with;
		}
		else//以寛适配这个不应该进来
		{
			m_fScaleX = m_fScaleY = (float)m_obScreenSize.height / f_4s_Height;
		}
...
CCSize  CCEGLViewProtocol::getVisibleSize() const
{
	...
	//add by sxin
	else if (m_eResolutionPolicy == kResolutionFixedStandard_4s)
	{	
		float f_4s_with = 960.0f;
		float f_4s_Height = 640.0f;

		float fheight = m_obScreenSize.width*(f_4s_Height/f_4s_with);

		return CCSizeMake(m_obScreenSize.width/m_fScaleX, fheight/m_fScaleY);		
	}	
	//end
    else 
    {
...
CCPoint CCEGLViewProtocol::getVisibleOrigin() const
{
	...
	{	
		...
		CCPoint VisibleOriginPoin = CCPointMake((m_obDesignResolutionSize.width - m_obScreenSize.width/m_fScaleX)/2, 
			(m_obDesignResolutionSize.height - fheight/m_fScaleY)/2);

		return VisibleOriginPoin;
